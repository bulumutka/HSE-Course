...
struct LibCppIsNoThrowConstructible<true, true, T, Arg> {
    using type = std::integral_constant<bool, noexcept(ImplicitCast<T>(Declval<Arg>()))>;
};
...
struct IsNoThrowConstructible
    : public LibCppIsNoThrowConstructible<IsConstructible<T, Args...>::value,
                                          std::is_reference_v<T>, T, Args...>::type {};

template <typename T, size_t N>
struct IsNoThrowConstructible<T[N]>
    : public LibCppIsNoThrowConstructible<IsConstructible<T>::value, std::is_reference_v<T>,
                                          T>::type {};
